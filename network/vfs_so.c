...
static int sub_init_signalling(char *so, int queue)
{
...
int vfs_signalling_thread(void *arg)
{
	...
	if (sub_init_signalling(argp->name, argp->queue))
	{
	...
	while (!stop)
	{
	    n = epoll_wait(epfd, pev, maxevent, 100);
	    for(i = 0; i < n; i++) 
		{
