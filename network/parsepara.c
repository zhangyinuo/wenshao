    ...
    const char *sName, char *sValue, size_t n)
{
	...
	snprintf(sValue, n, "%s", pPairList->pStrPairList[i].sSecond);
...
int main(int argc, char *argv[])
{
	...
	StringPairList *pList = CreateStringPairList(20);
	if (!pList)
	{
		fprintf(stderr, "CreateStringPairList %m\n");
		return -1;
	}

	if(DecodePara(argv[2], strlen(argv[2]), pList) != 0)
	{
		fprintf(stderr, "DecodePara %m\n");
		return -1;
	}
    
    int i;
    for(i = 0; i < pList->iLast; i++)
        fprintf(stdout, "%s=%s\n", pList->pStrPairList[i].sFirst,
                pList->pStrPairList[i].sSecond);
    return 0;
}
